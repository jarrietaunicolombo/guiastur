...
class ReleaseTurnoResponse
{
    ...
    public function __construct(
        ...
        GuiaReleaseTurnoDto $guia,
        AtencionReleaseTurnoDto $atencion,
        ...
        if ($guia === NULL) {
            throw new InvalidArgumentException("El Guia es requerido para Liberar el Turno");
        }

        if ($atencion === NULL) {
            throw new InvalidArgumentException("La Atencion es requerida para Liberar el Turno");
        }

        if ($fecha_registro === NULL) {
            throw new InvalidArgumentException("La Fecha de Registro es requerida para Liberar el Turno");
        }

        if ($usuario_registro === NULL) {
            throw new InvalidArgumentException("El Usuario que Registró es requerido para Liberar el Turno");
        }
    ...
    public function getGuia(): GuiaReleaseTurnoDto
    {
        return $this->guia;
    }

    public function getAtencion() : AtencionReleaseTurnoDto{
...
class GuiaReleaseTurnoDto{
    ...
    public function __construct(int $id, string $cedula, string $rnt, string $nombre, string $telefono = null, string $foto = null) {
        if($id === NULL || $id < 1) {
            throw new InvalidArgumentException("El Id del Usuario del Guia es requerida para Liberar el Turno");
        }
        
        if($cedula === NULL || empty(trim($cedula))) {
            throw new InvalidArgumentException("La Cedula del Guia es requerida para Liberar el Turno");
        }

        if($rnt === NULL || empty(trim($rnt))) {
            throw new InvalidArgumentException("El RNT del Guia es requerido para Liberar el Turno");
        }

        if($nombre === NULL || empty(trim($nombre))) {
            throw new InvalidArgumentException("El Nombre del Guia es requerido para Liberar el Turno");
        }
...
class AtencionReleaseTurnoDto{
    ...
    public function __construct(int $id, DateTime $fecha_inicio , DateTime $fecha_cierre = NULL, int $total_turnos) {
        if($id === NULL || $id < 1 ) {
            throw new InvalidArgumentException("El Id de la Atencion es requerido para Liberar el Turno");
        }

        if($fecha_inicio === NULL) {
            throw new InvalidArgumentException("La Fecha de Inicio de la Atencion es requerida para Liberar el Turno");
        }

        if($total_turnos === NULL || $total_turnos < 1) {
            throw new InvalidArgumentException("El Total Tornos de la Atencion es requerido para Liberar el Turno");
        }
