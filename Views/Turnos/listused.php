...
<body>
    ...
    <?php if (!$errorMessage && ($turnosResponse === null || @count($turnosResponse) < 1)): ?>
        <script type="text/javascript">
            window.onload = function () {
                showSimpleAlert("warning", "", "No existen turnos para En Uso en este momento");
            };
        </script>
    <?php endif; ?>
    ...
    <script>
        let url = '<?= UrlHelper::getUrlBase() ?>/Views/Turnos/index.php?action=usedtoday';
        let turnoStatus = "<?= TurnoStatusEnum::RELEASE ?>";
        let message = "Ha sido liberado el turno #";
        let action = "liberarturno";
    </script>
