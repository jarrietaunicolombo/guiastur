...
<body>
    ...
    <?php if (!$errorMessage && ($turnosResponse === null || @count($turnosResponse) < 1)): ?>
        <script type="text/javascript">
            window.onload = function () {
                showSimpleAlert("warning", "", "No existen turnos Liberados en este momento");
            };
        </script>
    <?php endif; ?>
    ...
    <script>
        let url = '<?= UrlHelper::getUrlBase() ?>/Views/Turnos/index.php?action=releasedtoday';
        let turnoStatus = "<?= TurnoStatusEnum::FINALIZED ?>";
        let message = "Ha sido finalizado el turno #";
        let action = "finalizarturno";
    </script>
